...
- Support for super-small displays: `rsact-tiny-icons` minimum icon size is 5px!
...
let icon = rsact_tiny_icons::icons_10::MAGNET;
```
...
Each set is an enum implementing `rsact_tiny_icons::IconSet` trait:
...
let icon10px = rsact_tiny_icons::common::icons_10::get_icon(kind);
```
