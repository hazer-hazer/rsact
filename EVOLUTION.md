...
- [] `FontProps` per-field reactivity (`Option<MaybeReactive<Font/FontSize/FontStyle>>`) is redundant with `Layout`'s reactive-on-write. Font is the only layout-affecting property not wired through `layout.setter(...)` — it uses `update_font_props`/`update_untracked` (see the `// TODO: Wrong ... Use setter!` in `widget/mod.rs`). Since a font change must re-run the monolithic layout memo anyway, per-field reactivity buys no granularity. Make `FontProps` fields plain (`Option<Font>` etc.) and route `.font()/.font_size()/.font_style()` through `layout().setter(...)` like `.width()/.padding()`. Folds two reactivity mechanisms into one and fixes the dead+broken font setters. Likely subsumed by the `font/mod.rs` `TextStyle`-widget TODO — do whichever lands first consistently.
