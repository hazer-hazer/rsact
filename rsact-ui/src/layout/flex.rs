...
pub fn model_flex(
    ...
    map!(move |block_model, parent_limits, size, wrap, gap, align, children| {
        let size = *size;
        let wrap = *wrap;
        let gap = *gap;
        let align = *align;
        ...
        for (item_index, child) in children.iter().copied().enumerate() {
            let (child_size, child_min_size) =
                (child.size.get(), child.min_size(ctx));
            ...
            if child_div_factors.main(axis) == 0 {
                ...
                let child_layout_size =
                    child_layout.with(|child_layout| child_layout.outer_size());
        ...
        for ((i, child), item) in
            children.iter().copied().enumerate().zip(items.iter())
        {
            let child_min_size = child.min_size(ctx);
            ...
            if child_div_factors.main(axis) != 0 {
                ...
                children_layouts[i] = model_layout(
                    ctx,
                    child,
                    Limits::new(child_min_size, child_max_size).inert().memo(),
                    size.inert().memo(),
                    // viewport,
                );
            }
            ...
            children_layouts[i] = children_layouts[i]
                .map(move |child_layout| child_layout.translated(next_pos));
        ...
        if !align.is_start() {
            for (child_layout, item) in children_layouts.iter_mut().zip(items) {
                ...
                let free_space = axis
                    ...
                    - axis.canon::<Size>(
                        line.used_main,
                        child_layout.with(LayoutModel::outer_size).cross(axis),
                    );
                *child_layout = child_layout.map(move |child_layout| {
                    child_layout.aligned(align, free_space)
                });
            }
        }
