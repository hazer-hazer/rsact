...
import { segments, seriesMax, shapes } from './chart.js';
...
describe('shapes', () => {
  ...
  it('emits an isolated point as a dot, not a polyline', () => {
    ...
    expect(polys.length).toBe(0); // two single-point runs
    expect(dots.length).toBe(2);
  });
  it('normalizes to the provided max (top of range hugs the top pad)', () => {
    const { polys } = shapes([50, 100], { ...opts, n: 2, max: 100 });
    // second point (v=100=max) → y at top pad (height - pad - full span) = pad
