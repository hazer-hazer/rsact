...
A pure, global pre-pass `collapse.ts`: over topo-ordered snapshots, a maximal run of consecutive commits whose **every tracked metric value is identical** to the run's first becomes one column, headed `<first8>..<last8>` (single-commit columns keep the plain `revLabel`). Global (shared column axis across all tables) to preserve #4 alignment. Produces the reduced snapshot/column list that `buildSeries` and all charts consume. **Default-on**, with an "expand all" toggle in the controls (state also in the URL, B5). Unit-tested: all-equal run collapses, any-change splits, gaps handled (a gap vs a value is a change), single-commit unaffected.
