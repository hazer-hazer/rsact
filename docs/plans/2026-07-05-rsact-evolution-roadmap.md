...
```text
6.4 tiled frame (a–d), one frame:
  collect pass  ─ tracked, probe-gated, NullRenderer ─▶ damage rects        (6.4c)
  merge overlaps (Rect::union) ─▶ chunk into row-bands ≤ budget            (6.4d)
  for each band: paint UNTRACKED, no probe, cull subtrees by outer ∩ band  (6.4b/c)
                 ─▶ hand the owned tile to the app ─▶ release ─▶ reuse     (6.7)
  mark probes clean after the LAST band; union new damage into next frame  (6.4c/6.7)
```
...
Order (resequenced 2026-08-01): 6.3c fill_contiguous FIRST — it is independent, it serves
the DrawTarget/simulator/golden path, and until it lands every SPI timing figure is
fiction. Then 6.4a measurement harness — it GATES 6.4d, so do not skip it to "save time";
its whole purpose is to replace the +14%/+49% estimates with numbers and to prove the
tile-invariance property. Then 6.4b culling (wins on Scrollables today, tiles or not),
6.4c collect/paint split, and only then 6.4d tiled output. 6.8 rotation designs inside
6.4d. 6.5/6.6/6.10 after.
...
1. **The ghost population is empty today — a precondition, not a detail.** `render_part` calls `self.shared.force_redraw.track()` _inside_ every probe poll body (`rsact-ui/src/el/render.rs:307`), so **every part probe has ≥1 source, forever, by construction**. Nothing can ghost until that page-wide edge dies — WS4.3's relocated singleton de-greed already names exactly this payoff ("every render probe currently tracks `force_redraw`, so that one demotion removes a page-wide fan-out edge per part"). WS6.2 introduced the non-reactive `full_flush` for the _flush_ decision, but `force_redraw`'s track edge survives as the per-part re-arm channel. Until it is gone, this idea cannot even be measured.
