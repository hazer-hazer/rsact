...
**Interactive view (2026-08-05):** this file is the source of truth; its browsable projection — workstream cards, decision gates, the issue inbox, copyable session prompts — lives in the repo as [`2026-07-05-rsact-evolution-roadmap.artifact.html`](./2026-07-05-rsact-evolution-roadmap.artifact.html) and is published (update **in place**, same URL forever) at <https://claude.ai/code/artifact/7a581012-6f6a-4515-9345-f1cb1ff0cf5b>. **A session that changes this markdown re-syncs that file in the same commit** — the sync procedure, the JS-data layout and the two easy-to-violate invariants are documented in a comment at the top of the HTML. It was committed because for ~30 sessions the artifact had no repo source, so each update meant re-fetching the published page, and it drifted several items behind the markdown every time.
...
- [ ] **5.5 The box/pixel rule: border moves to style, and the widget's layout copy dies (DECIDED 2026-08-07).** Raised by the maintainer while reviewing the built-widget layout: `LayoutData` is cloned from the builder into the retained widget, is **not reactive**, and drifts from the arena's authoritative copy — `widget/button.rs` has carried the admission for a while (*"a padding/border bound to a signal after build drifts here … accepted narrow limitation until render reads block_model off the arena"*). Audit of what that clone is for: **8 widgets carry it dead** (`canvas, flex, knob, label, select, show, slider, space`) and **6 read it** (`edge, checkbox, bar, button, container, scrollable`) — all through `block_model()`, and `Block::from_layout_style` **discards the padding** (`BlockModel { border_width, padding: _ }`). So an entire `LayoutData` per widget exists to deliver **one `u32`**.

  **The rule this exposed.** The split is not *style vs layout*, it is **does it change the box, or only the pixels inside it**. The codebase already followed it almost everywhere without saying so: `BorderStyle` carries `color`+`radius` and **no width**, while `OutlineStyle` carries `width` — because an outline does not inset content and a border does. Restating the rule positively is what makes the remaining cases decidable.

  **DECISION (maintainer, 2026-08-07): the SwiftUI model — border becomes fully style-only.** `border_width` leaves `BlockModel` entirely; the border is drawn with `StrokeAlignment::Inside`, i.e. **inside the widget's own rect, overlapping the padding ring**, and layout never learns about it. Rationale, in the maintainer's words: *"it is the most clear one and easier for users as they only need to add padding to avoid border overlapping content, while border width in layout loses styles pseudoclass capability much more needed."* The alternative — keeping the width in the box model and delivering it to render through the ctx — was recommended by the session and **rejected on that trade**: a layout-side width can never respond to `hovered`/`pressed`/`focused`, and that capability is worth more than automatic overlap avoidance. Note the trade is genuinely two-sided: a style-side width *cannot* be layout-affecting, because `get_style` resolves per pseudo-class and a hover-driven relayout would be layout thrash.

  **Work:** `BlockModel` loses `border_width` (and with it `full_padding()`, which collapses to `padding`); `BorderStyle` gains `width: u32`, mirroring `OutlineStyle::width`; `Block::from_layout_style` drops its block-model argument and takes the width from the style; the layout-side `border_width` builder and `Layout::set_border_width` go; `Button`'s default border moves from its `BlockModel` to `ButtonStyle`; each theme sets a default width in its single `fn border()`; then **`layout` is deleted from all 14 widgets**, which is the actual point — after it, the arena's `LayoutData` is the only place layout inputs live, there is nothing to drift, and reactive `.padding(signal)` starts working instead of silently going stale. Layout results change by construction (content is no longer inset by the border), so page goldens move — that IS the behaviour change, and it must be reviewed as one.

  **PROPOSITION (maintainer, 2026-08-07) — customizable stroke alignment, gated on ISSUE-3's `ext_draw` being computed.** `StrokeAlignment::Inside` is the default and needs nothing, because inside-aligned strokes paint within the box. `Center` and `Outside` paint beyond it by `w/2` and `w` respectively, so they are only expressible once a widget can *declare* how far outside its rect it paints — which is exactly `Widget::ext_draw() -> Padding` (WS6.4c(G), landed as an extension point returning zero). Until then the border stays `Inside`-only. **The same mechanism absorbs three other cases** the audit turned up, and the maintainer's decision is that none of them should be reserved in the box model either:

  | property | side | today | under the proposition |
  | --- | --- | --- | --- |
  | `BorderStyle::width` + alignment | style | Inside only | `Center`/`Outside` declare `ext_draw = w/2` / `w` |
  | `OutlineStyle::width` + `offset` | style | paints 1 px outside `outer` on every focused widget, undeclared | `ext_draw = offset + width` |
  | `scrollable.scrollbar_width` | style | **never seen by layout**; the scrollbar is painted over content, and the overlap changes with the theme (rgb 5 / binary 3) | declared via `ext_draw`, overlap accepted deliberately rather than silently |
  | `slider.thumb_size`, `track_width` | style | thumb/track are centred, so they escape the cross axis when larger than `inner` | declared via `ext_draw` |
...
**Status 2026-08-09 (design session — the tile is a budget):** **Maintainer redesign, adopted and measured — see 6.4d(7).** A tile stops being a `W × H` rectangle and becomes a **capacity in storage units**; the planner may emit any region fitting the buffer. Shape-bounding was rsact's own invention (ST7789 takes arbitrary rects; the panels that constrain geometry never need tiling; SPIM's `MAXCNT` is a byte count anyway). Measured: `tile-24` collapses onto `whole` on every row of `tile_plan_240.txt` — 2 regions → 1 and 6 → 1 — with the ten-band degenerate case unchanged. Notably it **deletes** the capacity-veto case the previous session measured at 8 ops against 5; that finding was an artifact of the invented constraint, and is kept in the docs as such. Also fixed the day before: containment was being vetoed and orphaning the inner rect (found by building a step-by-step explainer of the planner and running it on a widget-inside-its-ancestor case). Suites 75 / 114 / 123 / 8 / 8 / 78+5+4 / 19.

**Status 2026-08-10 (execution session — 6.4d renderer half + the ownership redesign):** **6.4d(5)(6) DONE, plus an ownership redesign the maintainer called mid-stream** (branch `ws6.4d-tile-renderer`, PR #47, not merged). The tile-backed renderer exists and is proven **pixel-identical** to a full framebuffer at ⅛ the RAM, through an N-buffered loop over a channel-shaped pool — the 112.5 KiB → <20 KiB acceptance target is now demonstrated rather than argued. What changed beyond the plan, all maintainer-directed: **(a) rsact no longer owns a renderer.** `W::Renderer` stays a `WidgetCtx` associated type (`El<W>` is `Box<dyn Widget<W>>`, `Canvas` holds `Box<dyn Fn(&mut W::Renderer)>` — the *type* is load-bearing); only ownership moved, and the caller lends one per call, which also infers it. `UI::new(stylist, viewport)` takes the size directly. **(b) `Renderer::SURFACE_UNITS`/`SURFACE_PIXELS_PER_UNIT` → `type Policy: FramePolicy`.** A renderer declares *how big a region you may ask me to paint* — answerable by a GPU and by an 11 KiB tile alike — and rsact learns nothing else about storage. The capacity check moved to `EGRenderer::attach`, the one place holding both the policy and the buffer: `const` for a fixed array, an assert at hand-off for a runtime-length slice. **(c) `FinishRender` and `RenderTarget` are both deleted** — a second seam. `Renderer` renders; flushing is the application's, and `RenderTarget` was only a re-declaration of `DrawTarget` for backends that have pixels at all. **(d) `attach`/`detach` are the primitives, `swap` is sugar** — not style: `swap` needs two surfaces at the instant of exchange, so a **single-buffer pool deadlocks** under it. **(e) attachment became a type-state** (`Attached`/`Detached` with an associated `Slot`, so the field itself vanishes rather than becoming `Option`): painting between a detach and the next attach was a logged no-op on the one path that runs every frame, and is now a compile error. **Three holes closed, each verified before being believed:** `PixelBuf for Box<[S]>` claimed `UNITS = usize::MAX` — the *no-constraint* sentinel — so **every heap surface bypassed the capacity proof** (an empty boxed slice satisfied `Tiles<240,240>` at compile time); `attach` wrapped every buffer as a tile, so a full-frame renderer that detached and reattached came back aimed at `Rect::zero()` and **flushed nothing from frame two onward**, invisibly to every op-log check; and `AsBytes` was a capacity claim about a buffer that could never be drawn into (removed — and it was the sole reason `PixelBuf`/`Surface` were two traits, now merged as `Framebuffer`). Also dissolved: the `Framebuf` **trait** (one real implementor) and `surface::Canvas` (its constructor allocated, which a renderer must not). **6.4e added as a blocker on the rest of WS6** — see the item. **Review round 2 (maintainer):** three more corrections. `UI::render` + `UI::with_damage` **deleted** — `Frame` is the only render API, because the whole-frame path never called `begin_region` and so **never aimed a tile-backed renderer** (verified: `drew = true` with the surface covering `0x0`, every flush sending nothing), and because under `Unbounded` `start_frame` plans the same damage into regions already paired with the rect to flush — one path, not two. `use_renderer` went with it; the benches, metrics probe and size probe drive a real `Frame` to completion now, which is both the true path and a better thing to measure (baselines unmoved). **The tiny-skia "shape not capacity" asymmetry is dissolved**, not documented: `Pixmap::take` + `from_vec` re-stride the storage, so `begin_region` reshapes it per region (`take` · `resize` · `from_vec`, inside the lent vector's capacity, no reallocation) and the bound is a byte budget exactly as for the framebuffer backend. A fixed pixmap pool under `Tiles<W, H>` now works, and the shape-bounded-`RegionLimits` follow-up this status line was going to need is **not required**. Also: `swap` no longer repeats the const capacity assert, which depends only on `B`/`C`/`P` and already fired at construction; only the runtime compare remains, and only because a runtime-length buffer's extent differs per value. Suites 75 / 114 / 123 / 9 / 9 / 77+5+5 / 19.

**Status 2026-08-08 (execution session — 6.4d, plan + driver half):** **6.4d(1)(2)(3) DONE** (branch `ws6.4d-tiled-output`, PR #45): the region planner, the type-level `FramePolicy` with its compile-time capacity proof, and the `Frame` driver. Measured on real damage, `tile_plan_240.txt`: **40–44× fewer required ops than bands** on the same change (6 vs 241, 5 vs 297, 16 vs 712), with `tile_invariance` holding on every planned schedule. Two **corrections to rules this plan stated**, both found by measurement rather than argued: (a) 6.4d(1)'s corollary *"overlapping rects must always merge"* is **false** — a 500-seed fuzz produced two rects sharing 6 px whose union is ×2.92 the sum of their areas, where merging costs more than the double paint it saves; what holds unconditionally is the sub-case that actually occurs, **containment always merges**, so planned regions may overlap and that is priced waste, not corruption; (b) a new **capacity veto** — a merge the surface cannot hold is never made, because chunking would immediately undo it and re-derive the split from the *union's* grid, slicing a widget that neither original rect split (measured 8 required ops against 5). Capacity outranks the region budget for the same reason. Also landed: `Renderer::SURFACE_PIXELS_PER_UNIT` so the capacity proof needs no `PackedColor` bound, `region_units` as the single spelling of the row-padded arithmetic, and **frame-coherence deferral** (6.7's "defer + union-coalesce, never abort") — dropping a `Frame` early re-plans its unpainted regions next frame instead of leaving stale rectangles. **REMAINING in 6.4d: the tile-backed renderer** — see (5) below; until it exists the driver runs against full-frame surfaces, so tiling buys traversal and transfer but not yet the 112.5 KiB → <20 KiB acceptance target. Suites 75 / 114 / 123 / 8 / 8 / 76+5+4 / 19.
...
- [ ] 6.4 **Partial-buffer (tiled) rendering** — **REDESIGNED 2026-08-01** (design session; supersedes the original "full / N-line strip / direct-to-target framebuffer modes" framing). Acceptance unchanged: 240×240 RGB565 heap 112.5 KiB → <20 KiB (D6 P1). Reference target: **ST7789/ST7735 SPI RGB565** (G3's color reference).
  ...
  **Cost model** (AA rasterizers deliberately excluded — they are acknowledged stubs, see backlog; centering the decision on them would be measuring the wrong thing). Per-pixel work is **invariant**: each output pixel is written once across the whole schedule. Only **per-object** work repeats — node visit, style resolve, primitive setup, text-run dispatch — and at ×1.6–2.0, not ×K: an object of height `h` over bands of height `s` is visited `1 + (h−1)/s` times, leaves dominate by count, and full-height containers are transparent no-ops. Estimated cold-frame cost **+14%** (100 nodes) to **+49%** (500 nodes); interactive frames **+0%** (damage fits one tile, no repetition at all). **MEASURED 2026-08-04 by 6.4a — the model holds where it was checkable, with two amendments.** Per-object ×1.42–2.17 at 10 regions (estimate was ×1.6–2.0 ✓) and per-pixel ×1.00 exactly ✓. Amendment 1: **traversal is worse than drawing** (×1.19–2.44), because "full-height containers are transparent no-ops" is true for ops and false for node visits. Amendment 2: **"interactive +0%" holds only for paint-only changes** — a text change is a full cold frame today (ISSUE-2). **Amendment 2 RETIRED 2026-08-07 (PR #44):** ISSUE-2 is closed, a text change now damages 1% of the viewport (93 ops vs 478), and "interactive +0%" covers text too. All three figures are the *culled* floor; uncalled, ×emit is the region count outright, which is why 6.4b became a precondition. Cold frames are **flush-bound** — 57600 px × 16 bit @40 MHz SPI ≈ 23 ms against 4–9 ms of paint — so with 6.7's ping-pong the paint hides inside the transfer and tiled is expected **~20% faster wall-clock** than a single framebuffer, which cannot pipeline paint against DMA (one buffer is both paint target and DMA source; double-buffering recovers the pipeline at 225 KiB). **These are estimates. 6.4a exists to replace them with measurements before 6.4d is committed.**
  ...
  - [~] **6.4.0 Prerequisites + interface spec (added 2026-08-04 — do these BEFORE 6.4a/b/c/d).**
    ...
    **Static dispatch confirmed; `dyn Renderer` costed and rejected.** Object safety is needed for the _widget_ tree (`El<W>` is `Box<dyn Widget<W>>`, `el/mod.rs:47`) and is already satisfied by `W::Renderer` being reachable from `W` — a generic `render<R>` method would make `Widget<W>` non-object-safe and the arena unbuildable, and `Canvas`'s `Box<dyn Fn(&mut W::Renderer))` (`canvas.rs:52`) cannot be written generically at all (`dyn for<R: Renderer> Fn(&mut R)` has no syntax). The erasure cost that decides it: `DrawTargetProxy::draw_iter` (`eg/renderer.rs:56`) calls `renderer.pixel()` **once per glyph pixel** and is the only path fonts use (`font/fixed.rs:122`, both the embedded-text and u8g2 arms), so `&mut dyn` would mean a vtable call per pixel with no inlining into the framebuffer store, on O(10⁴) covered pixels per text-heavy frame. Memory saving would be nil (one renderer variant per binary ⇒ no monomorphisation fan-out). **Escape hatch if runtime backend switching is ever wanted: a user-side `enum AnyRenderer` implementing `Renderer` by matching — ~40 lines, zero framework cost.** Consequence: **6.3c stays a pure optimization** (it was briefly a prerequisite under the erased design, and its signature is free to be `impl Iterator` again).
    ...
    - [x] **(ii-4) `NullRenderer<C>` generic over color** — the existing one is hard-wired to `type Color = NullColor` (`renderer.rs:220`). 6.4c's collect pass must run widget bodies against a no-op `Renderer<Color = W::Color>`, so a color-generic null renderer is a hard prerequisite for 6.4c. (`RecordingRenderer<C>` from 6.9 already has the right shape and may just serve, with its op log ignored.)
  ...
  - [x] **6.4c Collect/paint split — DONE 2026-08-07** (PRs #37 `RenderMode`, #38 the drawing seam, #39 the stacked-PR sync, #40 the traversal prune, #41 clip-aware hit-testing, #42 WS5.5 box/pixel rule). `RenderMode::{Fused, Collect, Paint}` with the renderer made **private** to `RenderCtx`, which itself implements `Renderer` — muting is an encapsulation boundary, not a capability asked of backends (all three of the original mute mechanisms were rejected by the maintainer, correctly: `set_muted` pushed our problem into user impls, zero-area was a hack, and a generic `RenderCtx` would have monomorphized the widget tree twice). `dyn Renderer` is impossible here (E0038 — the associated const `SURFACE_UNITS`), which is what forced the seam to be a concrete forwarding impl. Clipping became widget-behaviour-backed (`CLIPS_CHILDREN`/`CLIPS_SELF`), and its fallout was chased down rather than left: hit-testing (`hit = outer ∩ clips`, no `ext_draw` outset), the event-walk prune, and 6.11's no-op tiny-skia clip. Original text: — probes stop gating paint. `render_part` gains a mode: **Collect** (tracked, probe-gated, `NullRenderer`, pushes damage) and **Paint** (untracked, no probe, selected by geometry). The collect pass must genuinely run the body so dynamic dependencies re-track — an `is_dirty()` peek would freeze the recorded source set and silently break conditional reads. Paint passes must be **untracked** so `run_probe`'s `clear_sources` + re-subscribe + `mark_clean` round-trip is paid **once per frame regardless of tile count**; force-polling K times is K× graph churn (~24–48 ms at 8 tiles, comparable to the entire rest of the frame). Probes are marked clean **by the collect pass** (`run_probe` cleans after its closure runs) — **CORRECTED 2026-08-04, this item originally said "after the final paint pass", which is wrong and harmful**: deferring the clean until after the last paint swallows any write that lands mid-frame, so the torn region never re-plans and the tear becomes permanent instead of self-healing on the next frame. Cleaning in collect is what makes accepting class A/B skew reasonable at all. (The original wording is a leftover from when paint did the polling.) The damage sink stops clearing per pass (`page/mod.rs`) and unions across the flush boundary per 6.7's defer+coalesce. **Coherence invariant:** `tick()` between `begin_flush` and drain is a documented contract violation + `debug_assert`. Rationale: direct-to-tile makes the **widget tree** the frame snapshot, and a tree of live signals is not frozen unless something freezes it — 6.7 §8 defers new _frames_, but nothing otherwise stops a signal write between `next_tile` calls in an async app, which is the same generation-mixing §8 rejects arriving through a different door. (Copy-from-framebuffer would not have this hazard: there the framebuffer _is_ the snapshot. It is the price of decision 6.7-1.)
    ...
    **LANDING 2026-08-05 — PR #37 (seam) + PR #38 (modes), stacked; the traversal half is still open.**

    - **PR #37, (A) the seam.** `impl Renderer for RenderCtx<'_, W, CtxReady>` forwarding to a now-**private** `renderer` field; 19 call sites in 10 widget files; `FontHandler::draw`/`FontCtx::render` generic over the *renderer* (the font layer no longer depends on `el::ctx` at all); `render_font` + `render_focus_outline` routed through the seam; `Canvas`'s closure takes `&mut RenderCtx`. **Behaviour-neutral and proved so: every suite green with NOT ONE golden re-blessed.** Exactly one site in the workspace had to change for the Canvas type move — the test that annotated `&mut NullRenderer`.
    - **PR #38, (B) the modes.** `RenderMode::{Fused, Collect, Paint}` on `RenderShared`; the mute is one branch per forwarder plus `clear_outer` and the WS6.1 transparent-container clear (both write to the renderer directly and so must honour it by hand); `Collect`'s `clip_bounds() -> None`; `render_part` gains the geometry-selected Paint path (no probe, no damage) with the body extracted to `run_body` so the two paths cannot drift; `Page::collect` + `Page::paint_region` (the latter `untrack`ed) alongside the unchanged `use_renderer`. Six tests, **all verified to fail under inversion** — making Paint probe-gated fails three, un-muting Collect fails two. Suites 75 / **103** / **112** / 5 / 52+3+2 / 19, goldens still untouched.
    - **The 6.4a harness now drives the real pipeline**, not a simulation: `damage_after` calls `collect`, each region goes through `paint_region`, and `force_redraw` is gone from the region path. The goldens did not move when it switched over — which is both the evidence the old fake was faithful *and* a tile-invariance check of the actual 6.4c implementation.
    - **PR #40, (E)+(F)+(G) — 6.4c COMPLETE.** `WidgetFlags::{CLIPS_CHILDREN, CLIPS_SELF}` (declared by `Scrollable` / `Canvas`, pushed around the children loop / after the clear — never around the body, which would erase `Scrollable`'s own `outer`-drawn border); `clip_inner`, `ElState::clip_path` and `ClipPath` deleted; `Rect::outset` + `paint_bounds(layout, ext)` + `Widget::ext_draw() -> Padding` with all four consumers routed through it (ISSUE-3's fix, declared now and computed later); the prune itself on the composed `clip_bounds() ∩ region`, guarded by a `debug_assert` that a child may only escape a parent that clips.
      **Measured, not modelled: the prune hits the analytic floor EXACTLY on every page and schedule** — `measured == cullable`, asserted as an invariant so a careless re-bless cannot hide a regression. `×visit` 10.00 → **2.15** (rows-24), 5.00 → **1.54** (rows-48), 9.00 → **2.85** (grid-80); the scrollable page's *whole-frame* pass drops to **×0.48**, i.e. the prune makes even the single-pass full-frame path cheaper than before, because half its content is off-screen. The traversal term deferred out of 6.4b is closed. A `Page::nodes_visited()` counter (one `Cell` increment per processed node) is what makes it a measurement rather than an argument, and 6.6's dirty-list walk now has a number to beat.
    ...
    **`CanvasRenderCtx` — the documented future (maintainer request 2026-08-05, so it is not forgotten).** Passing `RenderCtx` is accepted *as a step*, not as the destination: `RenderCtx` is an internal pass context (probes, `part_probes`, `dirten`, the damage sink, `needs_redraw`, `RenderShared`) and user code must not reach any of it. The maintainer's framing: "I see all such types as context used in passes strictly internal … but Canvas closure should accept its own `CanvasRenderCtx` in the future … because Renderer API doesn't provide, expanding Canvas capabilities like adding rsact styles usage for example."
    ...
    impl<'a, W: WidgetCtx> CanvasRenderCtx<'a, W> {
        ...
        pub fn style<S: Style>(&self) -> S;               // rsact styles / theme colors — the stated motive
    ...
    **Mechanism (verified 2026-08-05 — both halves already exist, one dead and one live).** The *dead* half is `ElState::clip_path: Option<ClipPath>` (`state.rs:54`): `ClipPath` has exactly one variant, `InnerRect`, and `render_subtree`'s `Some(ClipPath::InnerRect)` arm already pushes a clip around the **whole subtree** — which is exactly the wanted behaviour — but the field is `None` from `for_builder` and set nowhere, so the arm is unreachable. The *live* half is `WidgetFlags: u8` (`el/flags.rs`), declared by `Build::flags()`/`Widget::flags()`, seeded into `ElState.flags` at build time and read by the framework (`maybe_hover` gates on `is_hoverable()`) — the same shape a clip declaration needs. So: add `CLIPS_CHILDREN` to `WidgetFlags` (bits 0–4 are taken and a commented-out `IS_EDGE` reserves `1 << 5`, so take the next free bit rather than colliding), have `Scrollable::flags()` set it, and have `render_subtree` read the flag instead of `state.clip_path` — then delete the vestigial field. A `bool` suffices because there is only ever one rect (`layout.inner`); `ClipPath`'s commented-out `Rect(Rect)` variant is the escape hatch if a widget ever wants a custom one. **Why a flag and not `ctx.clip_inner`:** the prune decision happens in `render_subtree` *before* the body runs, and `clip_inner` both runs too late and wraps only the widget's own drawing rather than its children. Regression test already exists: 6.4a's `count_escaping` must drop to 0 on the scrollable page once the flag is honoured.

    **CORRECTION 2026-08-05 — the dead arm pushes the clip in the WRONG PLACE, and reviving it as-is would break the widget that needs it.** `render_subtree`'s `Some(ClipPath::InnerRect)` arm wraps the *whole* `render_subtree_body`, which is `widget.render(ctx)` **and then** the children recursion. So it clips the widget's own paint too — including `render_part`'s `clear_outer()`, which fills `layout.outer`. Concretely, on the one widget that must set the flag: `Scrollable` draws `Block::from_layout_style(ctx.layout.outer, …)` (`scrollable.rs:266`), so a body-wide `inner` clip **erases its own background and border**, while the clear is trimmed to `inner` and the border ring keeps stale pixels. Latent today only because the arm is unreachable. So `CLIPS_CHILDREN` must push around the **children loop** in `render_subtree_body`, not around the body. Three rects, three consumers, and they are not interchangeable: the **clear** and the **widget's own decoration** need `outer` unclipped by self; the **children** need `inner`; and an **untrusted user closure** needs `inner` for an unrelated reason (see below).

    **`RenderCtx::clip_inner` is therefore DELETED (maintainer, 2026-08-05).** Its three call sites are `select.rs:386` (options, unimplemented), `scrollable.rs:350` (commented out) — both children-clipping, both subsumed by the flag — and `canvas.rs:97`, which is a *different kind* of need and moves into `CanvasRenderCtx` (clip on construction, pop on drop). Worth separating explicitly, because both push `inner` and the resemblance is misleading: Select/Scrollable assert "**my subtree** is confined", a structural fact about the tree that the prune can exploit; Canvas asserts "**this closure is not mine**", a sandbox around code the framework did not write, with no children involved. Only the first is information the traversal can use, so folding Canvas into the flag would be wrong even though the rect matches. `Renderer::push_clip`/`pop_clip` survive as the primitive with exactly two framework call sites (the children loop; `CanvasRenderCtx`). Side benefit: the "an early `Err` must not leave the clip stack unbalanced" hazard that `clip_inner`'s doc warns about stops being a widget-author concern, because widget authors can no longer reach the stack.

    **The two properties are duals, and their defaults are asymmetric on purpose.** `ext_draw` (G) grows a node's own bound *outward* ("I paint beyond my rect" — outline, shadow, later tooltips); `clips_children` caps the subtree's bound *inward* ("nothing below me paints beyond my rect" — scrollable, later overflow:hidden). The prune test with both is `paint_bounds(node, ext_draw) ∩ current_clip ∩ region ≠ ∅`, where `current_clip` is what `clip_bounds()` reports after every `CLIPS_CHILDREN` ancestor has narrowed it. `ext_draw` defaults to zero, which is **optimistic** — a widget that lies cracks a tile; `clips_children` defaults to false, which is **pessimistic** — a widget that lies only costs a wasted descent. That is the right way round, and it is why the seam's `debug_assert` guards the `ext_draw` side.
  ...
  - [ ] **6.4d Tiled output.** Strategy lives in the **output path**, orthogonal to `Renderer` — which stays "how to draw a primitive" and never learns tiles exist, seeing only a clip rect — and **absent from `WidgetCtx`**, so widgets, layout and event passes stay ignorant and the const generic never infects the type family every widget is generic over. `TiledOutput<C, const MAX: usize>`: const max sizes the backing array (alloc-free, no `Vec` — a step toward 6.4's no-alloc direction and WS18), runtime `budget ≤ MAX` plus `TilePolicy` knobs. **Buffer pool registered once** at UI construction (6.7 §11-Q3). Damage → **merge overlaps** (`Rect::union`, WS5.3 — overlap is common: a widget and its parent's `LayoutChange` root) → chunk each merged rect into **row-bands** ≤ `min(budget, peripheral_max_transfer)`; the latter is a hard ceiling independent of RAM (nRF52 SPIM `MAXCNT`) and only the app knows it. Full-screen guard: if merged damage ≈ viewport, one screen rect chunked into bands — classic strips, arrived at rather than designed in, and the guarantee that tiling is **never worse than a plain strip renderer**. Tiles held in **wire format** (big-endian RGB565: `solid_storage` returns a pre-swapped pattern so 6.3b's word-fill is unaffected, `pixel()` readback pays one `REV16`). Needs a **contiguous sub-buffer seam** — `draw_buffer` exposes only the whole buffer today (6.7 §9). Full-framebuffer mode stays, same entry point, different strategy (6.7 §11-Q2, settled by the encapsulation decision above).
    ...
    **AMENDED 2026-08-04 — region shape, and the interface moves to 6.4.0.** Three changes, all maintainer decisions.
    ...
    (9) **`full_frame_percent` REMOVED too — 2026-08-09.** Maintainer: *"basically looks like an early optimization baked by no cost proof. it is not what we need at early framework life stage."* Same category as (8) and it fails the same way. It collapsed the plan to one viewport-sized region once the regions painted ≥90% of a viewport's worth of pixels. Firing at ≥90% bounds the dead space it adds at `0.1·V` = 5760 px on 240×240, so to pay for itself it must remove `5760 / (F/p) ≈ 77–230` regions; under a tile policy the collapse *produces* about ten bands, so it could only ever remove regions if there were ninety-odd to begin with — at which point capacity chunking gives roughly ten anyway. **Measured** on four quadrants at 98.3% coverage: with an unbounded surface it changed **nothing** (the area test had already merged them into the viewport, ×1.017), and with a 5760-unit surface it spent 956 px of extra paint to save two regions — a loss at any plausible `F/p`. Redundant where harmless, harmful where it acts. The `plan_regions` pipeline is now **four steps** (clamp · merge · chunk · sort), one priced decision applied uniformly, and the "bands are arrived at, never designed in" story survives untouched: a viewport-sized damage rect still chunks into bands in the chunk step. **General lesson, now recorded twice:** a threshold on the *input* ("too many regions", "damage is basically everything") standing in front of a decision the cost model answers continuously is not an optimisation — it is a way to override the measurement without noticing.

    (8) **`max_regions` REMOVED and the merge threshold REOPENED — 2026-08-09, from a maintainer challenge ("it seems like an early optimization").** It was, and the plan had already said so: `max_regions` belongs to **6.5**'s `RegionPolicy` and was deferred there precisely because "ST7789/ST7735 have free rect addressing and would leave it unexercised and unvalidated". Pulling it into the planner as an optimisation was a mistake, and it is now provably not one. **Proof.** Cost a plan as `N·F + p·Σarea` (`F` = fixed per-region cost — `CASET`/`RASET`/`RAMWR`, DMA setup, `begin_region`/`end_region`, the tree descent, one flush call; `p` = per-pixel paint+transfer). Merging removes one region and adds `Δ` px of dead space, changing cost by `p·Δ − F`, so a merge wins iff **`Δ < F/p`**. The area test's job *is* that comparison, so at its fixpoint every surviving pair was already priced and rejected ⇒ any budget-forced merge changes cost by `p·Δ − F > 0`, **strictly worse, always**. Measured on six scattered changes: default → 6 regions / 1092 px (1.9% of screen); budget 4 → 9860 px (17.1%, ×9); budget 2 → 50600 px (87.8%, ×46). **DELETED 2026-08-09** (maintainer: *"there's no case when limiting to max regions would make sense, so why do we preserve this fake optimization? no-alloc is not an argument as rsact is tightly bound to alloc in places that are much more significant"* — correct on both counts). It was briefly kept as a bound on a finite list, by analogy with LVGL's `LV_INV_BUF_SIZE` — but that is the length of a fixed C array whose overflow behaviour is *invalidate the whole screen*, not a tuned knob, and rsact's damage list is a `Vec` in a crate that allocates a `Box<dyn Widget>` per node. The field, the forced-merge step, `cheapest_pair` and the `N` const parameter on `Tiles`/`Whole` are all gone. **Evidence it never mattered: `tile_plan_240.txt` did not move.** Note e-paper does **not** need this cap either: its `F` is hundreds of ms and its `p` tiny, so `F/p` exceeds the whole screen and a correctly-parameterised area test coalesces everything by itself — **a correct break-even subsumes every case the cap was imagined for, except the finite list**. Same category error found and fixed in `Whole<W,H>`, which forced `max_regions = 1` ("a GPU wants one walk, one scissor"): that is a *capacity* claim, not a region-count claim, and it measured at 88% of the screen repainted for six small changes. `Whole<W,H,N>` now takes a count like `Tiles`.

    **THE MERGE THRESHOLD IS NOW OPEN, and its SHAPE is the first question — do not just re-tune ×2.0.** The same cost model says the criterion is **absolute dead space** (`Δ < F/p`, a number of *pixels*) and not a ratio. A ratio scales the allowance with the size of the rects being merged, which nothing in the model justifies: two 16×16 rects may add 512 px under ×2.0, two 100×100 rects may add 20000 px — a third of a 240×240 screen — for the same one region saved. Rough figures for the reference target put the real allowance at **25–75 px** (RGB565 @40 MHz ≈ 0.4 µs/px; `F` ≈ 10–30 µs for an 11-byte command sequence + DMA setup + a ~3-node descent), i.e. an order of magnitude tighter than ×2.0. **Correction to (1): the roadmap cites ×2.0 as "LVGL's neighbourhood" — that is wrong.** Verified against `lv_refr.c` (v8.3 and master): `lv_refr_join_area` joins **only areas that overlap or touch** (`lv_area_is_on`) and **only when the union is strictly smaller than the sum** — effectively `Δ < 0`, i.e. LVGL treats `F` as negligible, which is defensible when you own the framebuffer and pay no per-region command sequence. **Maintainer's call (2026-08-09): find the best value by measurement, not argument.** What to build: a harness that varies the rule over a corpus of real damage sets and scores `N·F + p·Σarea` with `F`/`p` swept across plausible hardware (SPI clock × color depth × command overhead), reporting where the decision actually changes. The output is not one number but a `RegionPolicy` parameter per target — the same home `max_regions` belongs in.

    (7) **DECIDED 2026-08-09 (maintainer) — a tile is a CAPACITY, not a shape. Amends (1) and (5).** The surface constraint is a **unit count**: the user hands the renderer a byte buffer and rsact says where to blit it, so nothing about the buffer cares whether its 5760 units are laid out 240×24, 120×48 or 16×38. `Tiles<240, 24>` now reads *"a buffer big enough for a 240×24 tile"* and the planner may emit **any** region fitting that many units. Maintainer's argument, and it is the right one: bounding the *shape* was a constraint **rsact invented**, not one the hardware has — ST7789/ST7735 `CASET`/`RASET` take arbitrary column/row ranges, and a region strided at its own width is contiguous by construction, so any rect is one `set_window` + one DMA burst exactly like a band. The panels that *do* constrain geometry are precisely the ones that never need tiling (SSD1306's 8-row pages belong to a 128×64 mono display whose entire framebuffer is 1 KiB); e-paper's byte-aligned columns stay **6.5**'s `RegionPolicy`, which composes with a capacity bound rather than replacing it. Bonus alignment with the other ceiling: nRF52 SPIM's `MAXCNT` limits a transfer's **byte count**, which a capacity bound states directly where `W × H` could only approximate. **Measured** (`tile_plan_240.txt`, same buffer): `adjacent` 2 regions/6 visits → **1 region/5 visits**; `all` 6 regions/18 visits → **1 region/13 visits**; `tile-24` now matches `whole` on every row, i.e. the shape constraint was the *only* thing separating them. The degenerate case is unchanged — a collapsed full-screen region under 5760 units is still exactly ten 240×24 strips — so "never worse than a strip renderer" survives. **It also deletes (5)'s capacity-veto motivating case**: the 16×38 merge that shape-chunking sliced at y=24 for 8 ops against 5 simply *fits* (608 of 5760 units) and never happens. That measurement is kept in the module docs as a **recorded artifact** — it read as a subtle economic finding and was actually a self-inflicted constraint, which is the more useful lesson. The veto itself survives for genuine overflow, containment exemption intact. Two mechanical consequences: chunking is now **width-preserving** (`max_units / row_units(width)` rows per band — simpler than a 2D grid and it never introduces a vertical seam, so a widget can only ever be split horizontally), and `RegionLimits` carries `max_units` + `pixels_per_unit` instead of `max_region: Option<Size>`. **The compile-time capacity proof is UNCHANGED** — it already compared `region_units(MAX_W, MAX_H, ppu)` against `SURFACE_UNITS`, which is exactly the budget being declared.

    (5) **DONE 2026-08-08 — what the plan + driver half actually landed, and the two rules it corrected.** `rsact_render::region` is the planner (`plan_regions_into` into a caller-owned `Vec`, so the steady state allocates nothing); `FramePolicy` is a **type** (`Tiles<W, H, N>` / `Whole<W, H>`) whose largest region is an associated const, and `UI::start_frame::<P>()` proves it fits in a `const` block via `assert_policy_fits::<P>(R::SURFACE_UNITS, R::SURFACE_PIXELS_PER_UNIT)` — verified to have teeth by temporarily shrinking `RecordingRenderer::SURFACE_UNITS`, which yields `E0080` naming both renderer and policy. The proof lives in **rsact-render**, not rsact-ui, because `ci-test.sh` drives rsact-ui with `--lib` and its doctests would never run; the two `compile_fail` cases (one row too tall; a mono surface sized by area rather than padded rows) run in rsact-render's suite. `Renderer::SURFACE_PIXELS_PER_UNIT` (new, default 1) is what lets the proof avoid a `PackedColor` bound — that trait is the embedded-graphics backend's, and a GPU renderer should not have to name it to be checked. **Corrections:** (a) this item's "overlapping rects must always merge" is false — see the 2026-08-08 status line for the counterexample; the honest rule is *containment* always merges, and partial overlaps merge only when the area test says the union is cheaper, so **planned regions may overlap**. Consumers must not assume disjointness; painting is a pure function of position, so a twice-painted pixel is waste, never corruption. (b) The **capacity veto** is new and is not optional: `should_merge` rejects any union larger than `max_region` *before* consulting the area test, because a merge that chunking will immediately undo can only lose. Generalisation worth keeping: **chunking re-derives the split from the union's own origin, not from where the damage was** — so merging then chunking is not a no-op even when the region count is unchanged. **Deferral:** dropping a `Frame` with regions left folds them into the next frame's damage (`UI::deferred_regions`) rather than dropping them, since an unpainted region is a stale rectangle nothing else would repaint; deferred regions are re-planned, so they merge rather than accumulate. This is 6.7's frame-coherence rule at its smallest useful size, and the abort-vs-defer decision there is already taken.

    (6) **REMAINING: the tile-backed renderer** — the half that actually buys the RAM. `EGRenderer` still allocates `PackedFramebuf::new(viewport)`, i.e. a full-frame surface, so `Tiles<…>` today reduces traversal and transfer but not memory. What is needed, in the order the existing code suggests: **(a)** fold `EGRenderer` onto the `surface::Canvas<T>` helper PR #31 introduced (its `TODO` at `eg/renderer.rs` already says so, and the roadmap already said to fold it into 6.4d) — but note `Surface::new(size)` **constructs** its surface, which is exactly wrong for a user-*attached* buffer, so the trait needs an attach/detach shape first; **(b)** an origin offset applied on every canvas access, so absolute coordinates land in a smaller surface. Note the sign: `viewport_to_canvas`'s existing `point + area.top_left` is eg's `cropped` convention (viewport-local → canvas), whereas a tile needs `absolute − origin`. Getting this wrong produces a plausible image rather than an error, which is why constraint (b) below calls 6.4a's tile-invariance check the only real defence; **(c)** `EgTileRenderer::attach(buf)` / `detach() -> buf` as **inherent** methods, so no `type Surface` enters rsact and `W::Renderer` stays lifetime-free; **(d)** per-tile background initialisation, forced by `pixel_alpha`'s destination read — see constraint (b) below, which is now live rather than latent.

  **Final-sweep design constraints — all still apply:** (a) `PackedFramebuf`'s area-based packing asserts `area % pps == 0` — a real 122×250 mono e-paper **panics in the constructor** (30500 % 8 = 4), and rows are never byte-aligned (`eg/framebuf.rs`); minimal fix is to allocate `ceil(area/pps)` and drop the assert. **Moved to 6.5**, since the ST7789 reference target never hits it. **The per-row stride half of this constraint is now largely retired:** 6.3b found none is needed for a solid fill (whole words sit inside the row), and direct-to-tile retires the rest — a tile's own width _is_ its stride, so the sub-rect is contiguous by construction. Stride survives only for the optional zero-copy-from-full-framebuffer path; (b) `EGRenderer::pixel_alpha` blends against untranslated coordinates — dead today because `ViewportKind::Cropped` is never constructed, but **this goes LIVE with 6.4d**: a tile buffer is its own coordinate space, so the translation the bug was waiting for is precisely what direct-to-tile introduces. Also note `pixel_alpha` is a read-modify-write per pixel, which defeats write-combining; **and the RMW has a second, sharper consequence found 2026-08-04 — it constrains the tile protocol, not just performance.** `pixel_alpha` reads the **destination** (`eg/renderer.rs:173`: `canvas.pixel(p).map(|current| current.mix(blend, pixel.1))`) and the AA primitives use it heavily (`circle.rs:63-105`, `arc.rs:80-91`). Blending against an _ancestor's_ background is fine — the ancestor repaints earlier in the same pass, so painter order holds within a region. Blending against the **persistent frame** is not: full-frame rendering gets a correct backdrop for free because the framebuffer survives between frames (that is how AA edges currently compose under damage-driven partial redraw), whereas a tile buffer arrives holding whatever the previous tile left in it. So **every tile must be initialized to the true background before painting** — a full tile fill per tile, on top of the region's real work — and where a widget's backdrop is not opaque the tiled result _legitimately differs_ from the full-frame result, because "what is underneath" no longer exists. Note also `.unwrap_or(pixel.1)`: an out-of-bounds read silently yields the unblended color, so every mistake in this area produces a plausible image rather than an error, which is why 6.4a's tile-invariance op-log check is the only real defence; (c) decide the fate of the **layer dimension** first: two parallel `Layering` impls (`layer.rs` + a private copy in `EGRenderer`, TODO at `renderer.rs`), a per-draw BTreeMap lookup, and no code path that can create layer > 0. Sharpened: `Layer::fullscreen` allocates a **full-size framebuffer per layer**, so the latent memory multiplier is L × 112.5 KiB — tiling is what would make layers affordable at all — and `finish_frame_regions` streams layer-by-layer per region, multiplying per-region flush cost too. **✓ RESOLVED 2026-08-04 by PR #31 (`render: delete the layer dimension`, master `8810554`)** — the decision was _delete_: `layer.rs` is gone, `EGRenderer` holds a single `PackedFramebuf` + a `Vec<ViewportKind>` clip stack, `Viewport { layer, kind }` collapsed to a bare `ViewportKind`, and the per-draw `binary_search` for the layer index went with it. So the L × 112.5 KiB multiplier and the per-region layer loop are both retired, and 6.4b's "hoist the per-pixel `layers.binary_search` out of `draw_pixels`" sub-item is **already done**. Nested composition, if it is ever wanted, comes from tree-depth composition rather than a surface stack (rationale in the new `rsact-render/src/surface.rs`). **Follow-up left on master by #31 (not a blocker):** a `TODO` at `eg/renderer.rs` notes `EGRenderer` still holds `canvas` + `viewport_stack` inline instead of using the shared `surface::Canvas<T>` helper the same PR introduced — worth folding into 6.4d, which touches that struct anyway.
  ...
  **New invariant (2026-08-01):** any position-dependent effect — dithering, ordered halftone, gradients, pattern fills — must be a pure function of **absolute** coordinates, never tile-relative, or it seams at every tile boundary. Same class as "a primitive must not write outside its declared bounds", which multi-pass rendering turns from cosmetic sloppiness into visible cracks. 6.4a's tile-invariance check catches both.
  - [ ] **6.4e Unbind the framebuffer from embedded-graphics — BLOCKS the rest of WS6 (added 2026-08-10, maintainer).** `PackedFramebuf` is already generic enough to stand alone; `eg` is merely its only *current* user. Move it out of `eg/` into a top-level `rsact-render/src/framebuf.rs`, rename `PackedFramebuf` → **`Framebuf`** (it is now the only framebuffer type — the `Framebuf` *trait* was dissolved in WS6.4d(9), so the name is free and correct), and rename the `Framebuffer` trait → **`FramebufStorage`**, which is what it is about: capacity (`UNITS`, `unit_count`) plus access (`units`, `units_mut`) over a caller-owned buffer.

    **Why now, and why it blocks.** `EGRenderer` is going to split into (a) a straight embedded-graphics renderer **without** anti-aliasing and (b) a maintainer-written renderer with custom rasterisation **and** AA. Both need the same framebuffer. Doing the move *before* the split means one type serves both from day one; doing it after means the second renderer either reaches into `eg::` for its storage — encapsulating at the wrong seam and keeping the eg implementation messy — or re-derives its own, which is how two copies of the addressing arithmetic get born (the exact trap 6.4.0(i-2) already caught once, where the fast solid fill forked `y*width + x` and would have landed in the wrong row under a tile origin).

    **The seam is clean — verified 2026-08-10.** Pure, moves out: the `Framebuf` struct and every inherent method (`flat_index`, `row_stride`, `pixel`, `set_pixel`, `local_bounds`, `point_to_subpart`, `retarget`, `viewport`, `data`/`data_mut`, `capacity_units`, `into_buffer`, `new`/`tile`), the `FramebufStorage` trait and its `[T; N]` / `&mut [T]` / `Box<[T]>` impls, `units_for`, and `assert_region_fits`. Their signatures name nothing from embedded-graphics. Stays behind the feature, as `impl`s on a now-foreign-module type (legal — the type is still crate-local): `impl Dimensions`, `impl DrawTarget`, and the `PackedColor` impls for `Rgb555/565/666/888` + `BinaryColor`, which genuinely need `IntoStorage`, `PixelColor::Raw` and `RawU1`.

    **(a) The one thing that must NOT stay behind the feature: WS6.3b's fast `fill_solid`.** It is currently the body of `impl DrawTarget for PackedFramebuf`, but it is pure storage arithmetic — the per-row head/whole-words/tail split, `slice::fill` over whole storage words. If it stays in the eg impl, the future custom renderer inherits `Framebuf` **without** the 8–32× solid-fill win and re-derives it, re-forking the addressing. Make it an inherent `Framebuf::fill_solid(&mut self, Rect, C)` and have `DrawTarget::fill_solid` delegate. Same for `draw_iter`'s body, which is a `set_pixel` loop.

    **(b) `PackedColor` moves with it, keeping its name.** Its signature (`Storage`, `PPS`, `into_storage`, `as_color`, `set_color`, `solid_storage`) is eg-free, and it is the color half of the same abstraction — a framebuffer generic over storage is meaningless without it. The name still describes what it does (how a color packs into a storage word), so no rename. Only its impls stay behind `embedded-graphics`.

    **(c) The new module is unconditional**, not feature-gated: it pulls only `crate::color` and `crate::geometry`. With no backend feature enabled it compiles with nothing implementing `PackedColor`, which is harmless — and it is what lets the custom renderer use it without depending on embedded-graphics at all, which is the point of the exercise.

    **Out of scope, recorded so it is not re-litigated:** `Attached`/`Detached` + `Attachment<S>` stay in `renderer.rs`. They are about *renderers* holding surfaces, not about framebuffers — tiny-skia uses the same marker over a `Pixmap`, which is not a `Framebuf` and never will be.

    **What it unblocks, and one interaction to expect.** After the split, `EGRenderer` loses its `AA` type parameter entirely (it becomes the non-AA renderer), and `EgPrimitive::draw_aa` + `AntiAliasingEnabled` + the blended-edge halves of arc/circle/ellipse/polygon/rounded_rect migrate to the custom renderer. That touches `EgPrimitiveRenderer<C, AA>`, whose AA witness was deliberately restored in WS6.4d(9) — it is correct *today*, when one type has both modes, and becomes redundant when the mode is the renderer's identity. Do not remove it early on that argument.

    **Shape of the change:** a pure move + rename with **no behaviour change**, so it should land as its own PR and be reviewed as one — the WS6.9 goldens must come out byte-identical, and that is the acceptance test.
...
- [x] **6.11 tiny-skia clipping is a no-op — DONE 2026-08-06** (mask-based clipping; the WS6.4c clip work is what turned this from latent into visible, since `CLIPS_CHILDREN` pushes a real clip on every backend). tiny-skia has no scissor rect, so the clip is a cached `Mask` rebuilt only on push/pop (`w*h` bytes, `None` while Fullscreen) and passed to all **four** draw entry points, every one of which had `None` hard-coded: `fill_path`, `stroke_path`, `fill_rect`, `draw_pixmap`. Mask fill is deliberately **not** anti-aliased — a clip edge is a hard boundary, and AA would leak half-covered pixels outside it. 5 tests, 3 of which fail when the sites revert to `None`. Trap worth remembering: a fresh tiny-skia canvas is **opaque white**, so the first version of these tests asserted on `alpha != 0` and passed vacuously — the same white-on-white trap as the 6.4b golden. Original text: ** `clipped()` pushes `ViewportKind::Clipped` onto the layering stack (`tiny_skia/mod.rs:154-163`) but every draw path uses `Transform::identity()` and mask `None`, never consulting `current_viewport()` — Scrollable content overflows its bounds on this backend, and WS16.3 plans to build the desktop tier on it. Implement mask-based clipping (or transform+clip rect); precondition for WS16.3, verified by 6.9 goldens.
...
begin_region, color-generic NullRenderer), then the const-generic capacity chain, then
...
3. **The two-group split leaks if `Env` grows carelessly.** Every new field must be classified by the box/pixel rule at the point it is added, or the "color relayouts the page" bug arrives later. Encode it in the type (two structs, or a marker), not in a comment.
...
text color) moves no geometry and wakes no render probe, so it would relayout and repaint
...
**AUDIT EXTENDED 2026-08-07 (WS5.5) — `ext_draw` now has named customers, and they are all style-side.** The maintainer asked whether `border_width` was the only decorative property entangled with layout. It is the only one on the *layout* side; the audit found the traffic runs the other way, and WS5.5's decision routes all of it through this mechanism rather than through the box model:

| property | why it escapes | status |
| --- | --- | --- |
| `OutlineStyle::width` + `offset` | drawn `StrokeAlignment::Outside` at offset 0 + width 1 ⇒ 1 px beyond `outer` on every focused widget | structural, live today |
| `BorderStyle` alignment `Center`/`Outside` | does not exist yet — **gated on this item**, since a border that paints outside its box must be able to say so | WS5.5 proposition |
| `slider.thumb_size`, `slider.track_width` | thumb and track are centred on `inner`, so either escapes the cross axis once it exceeds it; both are **theme-settable**, so a theme can push a widget's paint outside its own box | live, style-dependent |
| `scrollable.scrollbar_width` | never seen by layout — painted over content, overlap varies by theme (rgb 5 / binary 3) | live, silent |

So `ext_draw` is not only the outline's fix: it is the declaration channel for **every style property with geometric reach**, which is what makes WS5.5's "keep it in style, declare the overhang" answer coherent instead of a dodge.
...
### ISSUE-4 — Style-property traits + a `Style` associated type: give styles the modifier vocabulary that layout already has

| | |
| --- | --- |
| **Filed** | 2026-08-07 (maintainer, reading Xilem — filed as an **early-stage idea**, explicitly not a proposal to implement) |
| **Kind** | idea / RFC — API ergonomics (the style half of the builder surface) |
| **Status** | **OPEN** — recorded, not scheduled. Blocks nothing in flight. If promoted it is a **design input to WS7.4**, not a new workstream. |
| **Area** | `rsact-ui` — `style/mod.rs` (`Style`, `WidgetStyleFn`, `declare_widget_style!`) · `style/stylist.rs` (`Stylist`/`InternalStylist`) · `widget/mod.rs` (the `LayoutWidget` + setter-trait family) · every `*Builder` |
| **Relates** | **G9** (DECIDED — TypeId styler registry; it *constrains the answer*, fact 6) · **WS7.4** (Stylist redesign — the execution site) · **WS5.5** (border moved layout → style the same week; fact 2 is its direct consequence) · **WS13** (the builder/widget split this rides on) · **ISSUE-3** (`Widget::ext_draw` — a second payoff, fact 7) · **H5** (themes as data) · **H7** (a declarative layer would compile to this vocabulary) · the standing `TODO` at `style/mod.rs:9-16` (the `StyleProp` idea — same target approached from the style side) · `EVOLUTION.md:31` (retire `MemoChain` for styles) |

**The submission.** Xilem is ahead of rsact on View↔Widget separation, and the shape worth stealing is that its **view tree is not the widget tree**: a view can consume a child, set one property on it, and vanish — `AlignedView { inner }` calls `horizontal_align(align)` on a child that has that layout property and never appears as a node. The nesting is arbitrarily deep and collapses bottom-up. Applied to styles: declare property traits (`BorderStyle`, `BackgroundStyle`, …) and each gets a view (`BorderStyleView` with `border_style` / `border_color` / …) contributing extension methods to *every* view whose widget has that property. For the traits to be implemented automatically rather than by hand, `Widget` needs a **`Style` associated type**, so the impl can be selected on "does this widget's style contain this property". Maintainer's own reservation, recorded verbatim: _"The problem I see now is that it may require to erase style types so button cannot then have explicit ButtonStyle."_

**What the code says today** (seven facts; the first three decide the shape, the rest decide the cost):

1. **rsact already implements this pattern — for layout — and it is the strongest argument for the idea.** `LayoutWidget<W>` (`widget/mod.rs:134`) is a bare capability trait exposing `layout_mut()`; `SizedWidget` (`:141`), `BlockModelWidget` (`:222`) and `FontSettingWidget` (`:249`) are default-method extension traits over it, opted into per builder with three empty impls (`button.rs:92-94`). `.width()` / `.padding()` / `.font_size()` are exactly the maintainer's collapsing views — they bind a property and dissolve. **The style half has no equivalent**: a widget gets one hand-written `.style(closure)` (`button.rs:81`) and nothing else. So the ask is not "import a Xilem concept", it is **"finish the symmetry the codebase already committed to"** — which is also AGENTS.md's standing convention (factor shared logic into a wrapper instead of copy-pasting it into every widget).
2. **WS5.5 just widened the gap by one property, which is what makes this timely rather than theoretical.** `border_width` used to be a `BlockModelWidget` *layout* setter — i.e. it had the ergonomic form — and WS5.5 moved it into the style, leaving that trait with exactly **one** method (`padding`). The in-code note is explicit that the border is now "a style property … set it through the widget's style (`.style(|s, _| s.border_width(1))`)". That is precisely the asymmetry this issue is about: a property that had a one-word modifier now needs a closure, for no reason other than which side of the line it landed on. WS5.5 was right on the merits (the border is decoration, per the box/pixel rule) — it is the *ergonomic* cost that this item would refund, and every future style property inherits the same cost until it does.
3. **DO NOT import the wrapper *type*, only the trait-organized vocabulary.** Xilem's modifiers must be nodes in the view tree because that tree is rebuilt and **diffed** each cycle; a modifier has to exist to participate. rsact has no diff — fine-grained reactivity replaced it — so a modifier can write the property once and cease to exist *at compile time*, which is strictly cheaper than a wrapper that has to collapse. rsact's `El<W>`/`View<W>` (`el/view.rs:23`) already borrows Xilem's `SingleViewMarker` trick (`:33`), so precedent for selective borrowing exists. **The erasure worry in the submission is a consequence of the wrapper, not of the idea** — see the next section.
4. **The property vocabulary already exists, but only inside a macro, not as a trait — and the census has the collisions in it already.** `declare_widget_style!` (`style/mod.rs:92`) is *role-typed*: a field declared `container` expands to `BlockStyle<C>` and emits `background_color` / `border_color` / `border_radius` / **`border_width`** (new in WS5.5) / `outline_{color,radius,offset,width}` (`:266-284`); a `color` field emits the setter plus a `transparent*` variant. `container: container` appears in **8 live widgets** — `ButtonStyle`, `ContainerStyle`, `CheckboxStyle`, `BarStyle`, `EdgeStyle`, `KnobStyle`, `ScrollableStyle`, `SelectStyle` (and commented-out in `flex.rs`) — so one blanket impl would reach all of them. But **two style types carry more than one field of the same role**: `SelectStyle` has `container` *and* `selected`, `SliderStyle` has `thumb` — and the macro disambiguates them by **renaming the generated methods** (`selected_border_color`, `thumb_color`). A trait cannot rename its methods. So a property trait must be keyed by **(property, role)**, not by property alone: the blanket impl covers the canonical `container` field only, and secondary blocks keep their bespoke macro-generated methods. `KnobStyle`'s `angle_start` + `angle` (two `Angle` fields, different meanings) is the same hazard one level down — and is precisely the objection already recorded against the `StyleProp` TODO at `style/mod.rs:9-16` ("dangerous default values … Knob start angle and sweep angle should not be the same"). **Verdict on that old TODO: adopt its trait-per-property half, reject its structural-by-type half.** The macro already knows each field's role, so it is the natural and near-free place to emit `impl HasBlock<C> for ButtonStyle<C>`.
5. **A `Style` associated type needs a "no style" inhabitant, and it costs the ability to withhold a modifier.** `Flex` has no style at all (its `declare_widget_style!` is commented out, `flex.rs`), as do `Space`, `Show`, `Dynamic` and `Canvas` — so `type Style` needs a `NoStyle`/`()` case, and `Stylist<NoStyle>` must exist or the bound must be conditional. Separately, `widget/mod.rs:138-140` records a deliberate design choice: _"Not implementing `SizedWidget` and `BlockModelWidget` … can be intentional to disallow user to set size or box model properties."_ A **blanket** `impl<T> BorderWidget for T where T::Style: HasBlock` removes that veto; per-builder opt-in impls (three empty lines, as today) keep it. The layout side chose opt-in; the style side should be asked the same question rather than inheriting a blanket by default.
6. **G9 forecloses erasure — the reservation is answered by a decision already taken.** G9 (DECIDED 2026-07-07) keys the styler registry on the **`TypeId` of the concrete style type**, and `get_style` (`el/render.rs:346`) resolves via `W::Stylist: Stylist<S>` on the concrete `S`. Erasing `ButtonStyle` would break both. Conversely the associated type *helps* G9 twice: `ctx.style::<S>()` stops needing the widget to re-name `S` at every call site, and G9 already kills `InternalStylist`'s hardcoded 11-style supertrait list (`style/stylist.rs:36-64`), which is the thing that currently makes a third-party widget unthemeable. **These two ideas are complements — this one should be evaluated *inside* WS7.4, not against it.**
7. **A second, unplanned payoff: ISSUE-3's `ext_draw` could be computed instead of declared.** `Widget::ext_draw` (`widget/mod.rs:74`) exists precisely because a widget's overstep is a *style* fact that the geometry gate cannot reach — its own doc says it must be "answerable without running the render body … which is why it lives here rather than being read off a resolved `BlockStyle`". ISSUE-3's WS5.5 audit extension then names its customers, and **all four are style-side**: outline `width`+`offset`, the prospective `BorderStyle` alignment `Center`/`Outside`, slider `thumb_size`/`track_width`, `scrollable.scrollbar_width`. With `Self::Style: HasBlock`, a **default** `ext_draw` could resolve the outline (and, once it exists, the border alignment) generically for all 8 block widgets, turning that part of ISSUE-3's postponed computation into a derived one; the slider and scrollbar rows argue the same case for role-keyed property traits beyond blocks. Caveat, stated honestly: it needs a signature that can see the stylist and pseudoclass, which `ext_draw(&self)` currently cannot.

**Does it force erasing `ButtonStyle`? No — that follows only from the wrapper *type*, which fact 3 already rejects.** A wrapper (`BorderStyleView { inner: Box<dyn ...> }`) must store *some* child uniformly, which is what pushes toward `dyn Style`. Extension methods on the builder need no uniform storage, so the concrete style survives:

```rust
// on the STYLE type — emitted by declare_widget_style! from the field's role (fact 4)
pub trait HasBlock<C: Color> { fn block_mut(&mut self) -> &mut BlockStyle<C>; }
impl<C: Color> HasBlock<C> for ButtonStyle<C> { /* &mut self.container */ }

// on the BUILDER — the capability trait, mirroring LayoutWidget::layout_mut
pub trait StyledWidget<W: WidgetCtx> {
    type Style: Style;                       // ButtonStyle<W::Color>, or NoStyle (fact 5)
    fn style_mut(&mut self) -> &mut WidgetStyleFn<Self::Style>;   // or an overlay, see below
}

// the modifier vocabulary — default methods, exactly like SizedWidget
pub trait BorderWidget<W: WidgetCtx>: StyledWidget<W>
where Self::Style: HasBlock<W::Color> {
    fn border_width(self, w: impl IntoMaybeReactive<u32>) -> Self { /* ... */ }
    fn border_color(self, c: impl IntoMaybeReactive<W::Color>) -> Self { /* ... */ }
}
```

`ButtonStyle` stays nominal, `Stylist<ButtonStyle<C>>` and G9's `TypeId` keying are untouched, and the associated type is on the **builder** — post-WS13 the builder *is* the view, and `.style()` already lives there (`button.rs:81`), so `Widget::Style` in the submission should read `Builder::Style` (the retained widget needs it only for payoff 7). Note what the sketch's first method is: `border_width` back in the form WS5.5 took it out of, without moving it back out of the style.

**The real cost is somewhere else, and this is the part that decides the idea.** `WidgetStyleFn<S> = Option<Box<dyn Fn(S, &StyleSelector) -> S>>` (`style/mod.rs:63`). If each modifier composes a new closure over the previous one, `.border_color(red).background_color(blue).border_radius(4)` is **three heap allocations and a three-deep dyn chain re-walked on every style resolve of every redrawing part** — versus the layout analog, where `.width()` writes a plain field. That is the wrong constant for a Blue Pill, and it is the difference between "finish the symmetry" and "import an allocation per modifier".

The way out is already in the data model: styles are `Copy` (the macro derives it) and `ColorStyle` is a **priority lattice** (`Unset` / `LowPriority` / `HighPriority` / `Transparent` / `Default*`, `rsact-render/src/style/mod.rs:15-28`) whose `set_high_priority` is exactly an overlay merge. So the builder can hold **one style-sized overlay value** that modifiers mutate in place, collapsed into a single closure at build — zero per-modifier allocation, one dyn call, and the merge semantics already exist. Two things that must be settled before believing it: (a) an eager overlay cannot be **selector-dependent**, so pseudo-class styling keeps the existing `.style(|base, sel| …)` escape hatch and the two compose (overlay first, then closure) — and note WS5.5 cites per-pseudo-class borders as a *reason* the border moved to style, so the escape hatch is load-bearing, not vestigial; (b) **where the reactive read happens decides the storage** — `.border_color(signal)` must be read inside the render part's probe poll to subscribe correctly, which means either the overlay stores `MaybeReactive` fields (fatter builder, fatter retained widget) or the read stays in the closure (back to per-modifier storage). Note `EVOLUTION.md:31` wants `MemoChain` gone from styles and "perfect reactive dependency style inheritance in render pass" — same question, already open.

**Bonus finding, cheap and independent of all the above.** The submission's own `AlignedView` example has a live wart on the *layout* side: `.horizontal_align()` / `.vertical_align()` are hand-written inherent methods on `ContainerBuilder` (`container.rs:53-79`) that call `layout.expect_container_mut()`, which is `unreachable!()` on the wrong kind (`layout/mod.rs:632-637`). Every other container-layout widget (`Button`, `Checkbox`, …) simply lacks the method. An `AlignWidget: LayoutWidget` trait would make the capability compile-time-checked, shared, and would delete a panic path — which the "UI must never panic" invariant wants anyway. **This is promotable on its own, without deciding anything above.**

**Verdict (recorded, not scheduled).** Worth keeping, and the reservation that prompted the filing is **resolved**: erasure is not required (and G9 forbids it) once the wrapper type is dropped in favour of builder-side extension traits — which is what rsact already does for layout. Do not open a workstream; fold it into **WS7.4** as a design input, where the Stylist is being reshaped anyway and where the two ideas share the same `S`-dispatch machinery. Promote when all four hold:

- (i) WS7.4 is being scheduled — landing this separately would reshape `declare_widget_style!` twice;
- (ii) the **overlay-vs-closure-chain** question is answered with a measurement, not taste — a `style_resolution` micro-bench is already G9's mandated tripwire, so the harness will exist;
- (iii) the reactive-read placement (`MaybeReactive` in the overlay vs read-in-closure) has a decided shape, co-decided with `EVOLUTION.md:31`;
...
The seven full analysis reports (with file:line evidence, alternatives considered, and per-direction open questions) were produced in the 2026-07-05 audit session. Their substance is condensed into this roadmap; the interactive version of this roadmap (browsable workstreams + copyable session prompts) is published as a Claude artifact at <https://claude.ai/code/artifact/7a581012-6f6a-4515-9345-f1cb1ff0cf5b>, and **its source is now in the repo** at [`2026-07-05-rsact-evolution-roadmap.artifact.html`](./2026-07-05-rsact-evolution-roadmap.artifact.html) (2026-08-05) — sync it in the same commit as any change here; procedure in that file's header comment.
