...
- [ ] 2.2 rsact-ui arena-owned probes (`ElState.part_probes` keyed by `&'static str` with the `PartId(u16)` TODO; `Page.render_probe`); borrow-choreography prototype first. **Prototype done (session 1) — recommendation recorded below; execute the rest in session 2.**
