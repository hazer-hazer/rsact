...
**STATUS 2026-07-07 — WS9a.1 DONE (single-item session; rest of 9a not started).** On `ws9-footprint-diet`. Commits: `dfaf2b5` (size-probe WS2 bit-rot fix — prerequisite to measure), `fb04eda` (9a.1 EffectQueue). Both wins verified and exceeded estimates; full reactive suite unchanged (**72 pass / 1 known-fail** `static_wrapper` = WS4), feature-powerset green (72/72, incl. `debug-info`), dependents green (metrics-probe 3/0, rsact-ui lib builds). Layer-2 baseline row re-recorded from the fixed tool (see Baselines).
