...
- **The reactive core is genuinely small on target:** 16.8 KiB `.text` on a real thumbv6m build, ~20 B of statics, **zero steady-state allocations** after warm-up.
...
```
            ┌─────────────────────────────────────────────────────────────┐
 LAYER 0    │ WS0 buildable+measurable   WS1 core correctness quick wins  │  ← start both now, parallel
            └───────────────┬──────────────────────┬──────────────────────┘
                            │                      │
 LAYER 1    ┌───────────────▼──────────┐  ┌────────▼─────────────────────┐
 (engine)   │ WS2 observer/render      │  │ WS9a engine diet: collections │
 ...
 (pipeline) │ WS5 layout off-graph +   │──▶│ WS6 damage-driven rendering  │
            ...
            └──────────────────────────┘   └──────────────────────────────┘
                    │
 LAYER 4    ┌───────▼──────────────────────────────┐  ┌───────────────────┐
 (surface)  │ WS7 API collapse (events, WidgetCtx, │  │ WS8 primitives     │
            │     Stylist, Widget trait) — BREAKING│  │ WS10 platform layer│
            └──────────────────────────────────────┘  └───────────────────┘
                    │
 LAYER 5    ┌───────▼──────────────────────────────┐
 (exterior) │ WS11 polish: examples, docs, naming  │   ← LAST, by design
            └──────────────────────────────────────┘
```

**Suggested execution order:** WS0 ∥ WS1 → WS2 → (WS3 ∥ WS4 ∥ WS9a) → WS5 → WS6 → WS7 → (WS8 ∥ WS10 ∥ WS9b) → WS11.
WS7's _decisions_ are locked at Gate time (now); only its _execution_ is late. Cheap de-genericization (Event::Custom deletion, `Dir`/`V` params) may ride along with WS2/WS4 if convenient — they're zero-user-impact.
...
- [x] **G6 (gates WS2/WS8) — DECIDED (2026-07-06): delete the registry from the render path**; the polled primitive becomes the owned **`Probe`** handle (see WS2). Public keyed `observe()` is deprecated (kept at most as a thin compat wrapper during migration; final removal call at WS2 execution). `ahash` leaves the no_std build with it.
...
- [ ] **0.1 no_std f32 math via `FloatExt` re-export (decided 2026-07-06):** rsact-render gets two mutually-exclusive math features — **`libm` (default)** → `pub use num_traits::Float as FloatExt;` (forward `num-traits/libm`) and **`micromath` (opt-in)** → `pub use micromath::F32Ext as FloatExt;` (faster approximations at accuracy cost — the _user_ decides; micromath 2.1's `F32Ext`, incl. `sin_cos`, is drop-in std-compatible — maintainer-verified). `compile_error!` when both/neither (same pattern as the storage backends). Primitives (`eg/primitives/sector.rs`, `arc.rs`, `line.rs`, siblings) just `use crate::FloatExt as _;` — on std builds inherent `f32` methods shadow the trait automatically, so the simulator uses std math with zero cfg. Acceptance: `cargo build -p rsact-render --no-default-features --features embedded-graphics,libm --target thumbv7m-none-eabi` succeeds (+ same with `micromath`; + thumbv6m compile-check).
- [ ] **0.2 portable-atomic, plain (decided 2026-07-06):** replace `FONT_UNIQUE_ID`'s `core::sync::atomic` use (`rsact-ui/src/font/mod.rs:289`) with `portable-atomic` types, **no rsact feature wiring**: on thumbv7m+ it compiles to native instructions; on thumbv6m the _end product_ selects the fallback itself — via feature unification on its own `portable-atomic` dep (`features = ["critical-section"]`) or `--cfg portable_atomic_unsafe_assume_single_core` in RUSTFLAGS. Document both in the README's thumbv6m note. Acceptance: rsact-ui builds for thumbv7m with `unsafe-single-thread,embedded-graphics`; thumbv6m compile-check documented.
- [ ] **0.3 metrics contract, local-first (decided 2026-07-06):** one command (`cargo run -p metrics-probe -- record` / `-- diff <rev|file>`) emitting a per-commit JSON snapshot — node counts by kind (**extend `current_runtime_profile()` to count Probes/Observers**, `runtime.rs:1424-1431` hides them today), live heap bytes, allocs/frame (idle + change), layout counters (0.5), `.text/.rodata/.bss` per target — stored locally, keyed by `git rev-parse HEAD`, plus a static HTML viewer over the snapshot dir. **CI runs the same tool** and merely archives snapshots + posts PR delta comments — remote mirrors the local tool, never replaces it. Adopt the audit's probe crates (reactive-only bin; 5- and 10-widget UI bins) as the measured binaries. Baseline row: reactive core = 16.8 KiB `.text` (thumbv6m), ~56 B statics.
- [ ] **0.4 node-count regression test**: canonical static page asserts node counts + idle-frame allocs through the 0.3 snapshot (today: 42 nodes / 10 labels, 0 idle allocs — lock both).
- [ ] **0.5 layout counters**: `#[cfg(feature = "layout-counters")]` visit/measure counters in `layout/model.rs`/`flex.rs` + a baseline-locking test (today: ~180 visits / 60–120 measures for one label change on a 30-node page) + criterion benches `layout_full`/`layout_leaf_change`; counters feed the 0.3 snapshot.
- [ ] **0.6 workspace feature audit (expanded 2026-07-06):** a systematic pass, not just the found leaks — every optional dep behind `dep?/feature` syntax; per-crate defaults minimal; features propagate top-down through the crate tree (root → ui → render → reactive) with nothing extra enabled by default; feature-matrix doc table; `cargo hack --feature-powerset` green; `cargo tree` of the minimal profile shows zero unexpected entries. Known targets found by the audit: `std → tiny-skia/png-format` leak (missing `?`), forced workspace `embedded-graphics/defmt`, unused workspace `micromath`/`smallvec`.
...
**Sessions:** 1 · **Risk:** medium · **Directions:** D2, D7, D1 · **Depends on:** WS1 (1.1), WS2 (arena-owned observers) · **Gated by:** G11.
...
- [ ] **3.0b Full disposal audit of rsact-ui**: inventory every `create_signal`/`create_memo`/`create_effect`/`Layout` creation site (widgets, page, ui, event); record who _should_ own each; one lifecycle test per path (page drop, `set_children` subtree replace, `Dynamic` rebuild, navigation round-trip) asserting counts return to baseline. **Answer on record to the maintainer's question ("do widget-stored signals get disposed?"): no — nothing disposes them today.** Widget structs hold `Copy` handles; dropping `ElData` drops 8-byte keys while the runtime nodes live forever. The scope model is the fix: widgets are _built_ inside a scope (3.1 per-page, 3.2 per-subtree), so build-time `create_*` calls are scope-owned and die with it; the widget's now-dangling handles are safe post-disposal (the element is gone; any straggler read becomes a logged no-op via WS1.8 `try_*`). This audit's inventory is the direct design input for 3.2.
...
rsact-ui de-greed (4.2 verification sweep, 4.3 singletons, 4.5 widget audit). 4.4 is an
investigation — do NOT land a demotion mechanism without resolving the conditional-
first-run hazard (options recorded in the item). Re-baseline the WS0 regression numbers
and record old→new in the roadmap.
```
...
Acceptance (D3's criteria): text update in a Fixed×Fixed label = 1 node visit, ≤2 measures, only that label's observer re-runs; show-toggle re-solves only the parent flex; root change ≤ +10% of today; per-node retained state ≤ 64 B, compiled out under default features; existing layout/UI tests green.
...
counters and WS4's Layout/MaybeReactive state. Run 5.0 first as its own commit series
...
- [ ] 6.3 **`FinishRender` regions API** + row-contiguous output: `finish_frame_regions(target, &[Rect])` using `fill_contiguous`/scanline runs instead of the per-pixel iterator (likely 5–10× SPI win even before partial flush — D4 measured path).
- [ ] 6.4 **Framebuffer modes** in rsact-render: full (current), N-line strip rendering, direct-to-target; user-provided buffer. Acceptance: 240×240 RGB565 heap from ~128 KiB → <20 KiB in strip mode (D6 P1 target).
...
landed (arena-owned observers; changed-set API). Sequence: 6.1 → 6.2 → 6.3 first (correct-
ness + immediate SPI win), then 6.4 framebuffer modes, then 6.5/6.6. The invariant from
...
- [ ] 7.1 **Events (G5: `Event::Custom` KEPT)**: custom events remain the extension point for app-defined widgets (user widgets are concrete over their own ctx and can match `Event::Custom(MyEvent::…)`). Scope reduces to: the `MoveEvent`→`InputEdge` rename TODO, reserving room for future `Key(u8)`/`Char`, and making `E = ()` the zero-ceremony default (via G4's blanket ctx impl) so apps without custom events never have to name it.
...
- [ ] 7.5 **`WidgetCtx` → two associated types (shape pending G4 sign-off)**: `WidgetCtx { type Renderer; type Event; type Color; }` where `Color` is set from the renderer's color in every impl (kept so `W::Color` keeps compiling), with a blanket `impl<R: Renderer> WidgetCtx for R { type Event = (); }` — `type W = MyRenderer` works with zero ceremony; apps with custom events write one small ctx impl. Existing `impl<W: WidgetCtx> Widget<W>` code compiles **unmodified**; delete `Wtf` + the `PhantomData` plumbing. Optional `default-backend` feature exporting `type W` sugar (D7-friendly).
...
Acceptance: one (or two) clearly-labeled breaking releases; UI suite green; a widget file compiles unmodified across 7.5 (proof of the alias-shim claim); size-probe delta ≈ 0 for 7.5 (expectation-setting) and −10–20% rsact_ui `.text` for 7.2.

**Session prompt:**

```
Read docs/plans/2026-07-05-rsact-evolution-roadmap.md — WS7 + gates G4/G5/G9 (their
recorded answers). Verify WS2/WS4/WS5 landed. Execute the staged collapse 7.1→7.7 in
order (each stage is sed-able and independently testable; 7.1/7.2 may already have
landed early — check the checkboxes). Batch into at most two breaking releases. The
compat claim to preserve: existing `impl<W: WidgetCtx> Widget<W>` blocks keep compiling
through 7.5 via the alias trait.
```
...
- [ ] 9a.4 Logging policy per G12 (`max_level_off` in release-embedded or defmt); `WidgetFlags` → bitflags; drop `itertools` (4 call sites); replace/retire `derivative` (with WS7.4).
...
- [ ] 10.2 **Focus navigation reinstatement** (G10): tree-order traversal over the arena on PageState; wire `interpret_as_focus_move`; make `auto_focus` real; retire the absolute-index model and the dead `event/select.rs` stub. Acceptance: the 3d_printer page fully drivable with Move/Press only.
- [ ] 10.3 **UiQueue user messages**: `UiMessage::Custom(M)` + `on_message` hook (driver-level, never entering `Widget` — per G5's model).
- [ ] 10.4 **Input-driver layer**: `trait InputSource` + reference quadrature-encoder (debounce/accel) and button impls; one embassy/RTIC example compile-checked in CI.
...
```
Read docs/plans/2026-07-05-rsact-evolution-roadmap.md — WS10 + gates G7/G10. Verify WS3
landed (stale-ElId pruning) before 10.2. 10.1 is rsact-reactive + docs; 10.2–10.6 are
rsact-ui. Acceptance for 10.2: a test drives a page top-to-bottom with encoder events
only (Move/Press), focus visibly traverses in tree order.
```
...
- [ ] 11.7 Size/RAM numbers in README from the WS0 CI (the LVGL-comparison headline; target claim per the decided floor: a 10-widget mono UI fits the Blue Pill — framework ≤ ~48 KiB flash, ≤ 20 KiB RAM total).
...
- **Command-buffer rendering + diff** (D2 opt 2 / D5 alt b): RAM-prohibitive on MCU; the right future for a desktop/`std` tier and the only justification for multi-renderer binaries — keep the `DrawCommand` vocabulary in `canvas.rs` as seed.
...
- **No feature-flag sprawl:** the only sanctioned axes are storage backend (std | single-thread | unsafe-single-thread), render backend, font provider, extras (simulator/anim/tiny-icons/debug-info), plus `incremental-layout`/`layout-counters` while maturing. Anything else must be architectural (pay-per-use by construction).
...
## Baselines & verification commands (as of 2026-07-05, working tree at dc9bf3e+dirty)
...
| Static UI cost                           | ~3 nodes / ~1.7 KB heap per label; 10-widget page = 46 nodes, **31 inert** (→ WS4 target: ≈0 nodes)                                        |
